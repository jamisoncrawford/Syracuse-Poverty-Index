...
v <- "0.14.2"       # Assign version number
...
sets <- paste0("Poverty Index Datasets ", v, ".xlsx")
ccdi <- c("text", "text", "text", "date", "numeric")
...
pops <- tibble(date = rep(dates, each = 55),
...
tidy <- tidy %>%
    mutate(child_lead_aprc   = round(chlp, 3) * 100,
...
write_csv(tidy, paste0("index_", v, ".csv"))

tidy %>% select(Month:Properties, `Child Lead Poison Rate`:`Index Score`) %>%
  write_csv(paste0("index_", v, "_redact.csv"))
...
write_csv(tidier, paste0("index_", v, "_tidy.csv"))
...
write_csv(tidiest, paste0("index_", v, "_tidy_percents.csv"))
...
write_csv(tidy_raw, paste0("index_", v, "_tidy_raw.csv"))
